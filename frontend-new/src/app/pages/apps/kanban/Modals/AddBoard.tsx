...
const schema = Yup.object({
  name: Yup.string().required("لطفاً نام برد را وارد کنید"),
  columns: Yup.array().of(
    Yup.object().shape({
      name: Yup.string().required("لطفاً نام ستون را وارد کنید"),
      color: Yup.string().required("لطفاً یک رنگ انتخاب کنید").oneOf(colors),
    }),
  ),
  isPrivate: Yup.bool(),
});
...
function AddBoardForm({ close }: { close: () => void }) {
  ...
  const {
    ...
    defaultValues: {
      name: "",
      columns: [
        { name: "در حال انجام", color: "primary" },
        { name: "انجام شده", color: "success" },
      ],
      isPrivate: false,
    },
  });
  ...
  return (
    <form className="pt-4" onSubmit={handleSubmit(onSubmit)} autoComplete="off">
      ...
      <ul className="mt-4 flex flex-col">
        {fields.map((item, index) => (
          <li key={item.id}>
            <Input
              label="نام ستون:"
              placeholder="نام ستون را وارد کنید"
      ...
      <div className="mt-4">
        <Switch label="برد خصوصی" {...register("isPrivate")} />
      </div>
