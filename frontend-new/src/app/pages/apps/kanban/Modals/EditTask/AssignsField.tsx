...
import { allMembers, Member } from "../../data";

// ----------------------------------------------------------------------

export function AssignsField({
  onChange,
  value,
  name,
}: {
  ...
  const removeItem = (uid: string) => {
    const newValue = value?.filter((item: { uid: string }) => item.uid !== uid);
    onChange(newValue || []);
  };
                ...
                value.map((member) => (
                  <li key={member.uid}>
                    ...
                    >
                      ...
                      />
                      <span className="mx-2">{member.name}</span>
                    </Tag>
                  </li>
                ))
              ) : (
