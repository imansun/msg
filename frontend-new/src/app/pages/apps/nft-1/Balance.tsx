...
export function Balance() {
  return (
    <Box className="rounded-lg bg-linear-to-br from-purple-500 to-indigo-600 px-4 pb-4">
      <div className="pt-3">
        <h2 className="text-sm-plus font-medium tracking-wide text-white">
          موجودی شما
        </h2>
      </div>
      <div className="mt-2 flex w-9/12 min-w-0 items-center gap-1">
        ...
        <CopyButton value="0x9CDBC28F0A6C13BB42ACBD3A3B366BFCAB07B8B1">
          {({ copied, copy }) => {
            if (copied) toast.success("آدرس کیف پول شما کپی شد");
            return (
              <Button
                unstyled
                onClick={copy}
      ...
      <div className="mt-4 text-3xl font-semibold text-white">۲۷۸,۳۰۰,۰۰۰ تومان</div>
      <div className="mt-3">
        <p className="text-xs text-white/90">پیشنهادها</p>
        <div className="flex justify-between">
          <span className="font-medium text-white">۱,۷۵۰,۰۰۰,۰۰۰ تومان</span>
          <span className="text-right text-xs text-white/90">
            <span className="align-sub">(۵۵٪)</span>{" "}
