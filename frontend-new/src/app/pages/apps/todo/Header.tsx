      ...
      <header className="flex items-center justify-between">
        <div>
          <div className="flex gap-2">
            <p className="dark:text-dark-50 text-xl font-medium text-gray-800">
              روز من
            </p>
            <ActionMenu />
          </div>
          <p className="mt-1 text-xs">
            {moment(now).locale(locale).format("dddd, DD MMM.")}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Input
            ...
            classNames={{ root: "max-md:hidden", input: "text-xs-plus h-8" }}
  ...
  return (
    <Menu as="div" className="relative inline-block text-start">
      ...
      <Transition
        ...
        className="dark:border-dark-500 dark:bg-dark-700 absolute z-100 mt-1.5 min-w-[11rem] rounded-lg border border-gray-300 bg-white py-1 font-medium shadow-lg shadow-gray-200/50 outline-hidden focus-visible:outline-hidden ltr:left-0 rtl:right-0 dark:shadow-none"
      >
        <MenuItem>
          {({ focus }) => (
            <button
