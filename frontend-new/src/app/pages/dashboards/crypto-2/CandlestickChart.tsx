...
const chartConfig: ApexOptions = {
  ...
  tooltip: {
    custom: function({ seriesIndex, dataPointIndex, w }) {
      ...
      return `
        <div style="padding:6px; font-size:13px; text-align:right; direction:rtl;">
          <div><b>باز شدن:</b> ${formatDollarToToman(ohlc[0])}</div>
          <div><b>بیشترین:</b> ${formatDollarToToman(ohlc[1])}</div>
          <div><b>کمترین:</b> ${formatDollarToToman(ohlc[2])}</div>
          <div><b>بسته شدن:</b> ${formatDollarToToman(ohlc[3])}</div>
        </div>
      `;
    }
  },    
