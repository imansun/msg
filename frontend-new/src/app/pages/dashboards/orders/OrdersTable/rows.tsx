...
export function DateCell({ getValue }: { getValue: Getter<any> }) {
  ...
  const date = moment(Number(timestapms)).locale(locale).format("DD MMM YYYY");
  const time = moment(Number(timestapms)).locale(locale).format("hh:mm A");
  return (
    <>
...
export function TotalCell({ getValue }: { getValue: Getter<any> }) {
  return (
    <p className="text-sm-plus dark:text-dark-100 font-medium text-gray-800">
      {formatDollarToToman(getValue().toFixed(1))}
    </p>
  );
}
