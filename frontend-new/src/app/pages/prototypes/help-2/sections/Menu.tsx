// Import Dependencies
import {
  ArrowTrendingUpIcon,
  DevicePhoneMobileIcon,
  ShieldCheckIcon,
  UserIcon,
} from "@heroicons/react/24/outline";
import clsx from "clsx";

// Local Imports
import { Box } from "@/components/ui";

// ----------------------------------------------------------------------

interface MenuItem {
  id: string;
  Icon: React.ComponentType<{ className?: string }>;
  title: string;
  description: string;
  isActive: boolean;
}

const items: MenuItem[] = [
  {
    id: "start",
    Icon: ArrowTrendingUpIcon,
    title: "شروع کار",
    description: "راهنمای شروع استفاده از سامانه و امکانات اولیه.",
    isActive: true,
  },
  {
    id: "my-account",
    Icon: UserIcon,
    title: "حساب کاربری من",
    description: "مدیریت اطلاعات حساب کاربری و تنظیمات شخصی.",
    isActive: false,
  },
  {
    id: "mobile-app",
    Icon: DevicePhoneMobileIcon,
    title: "اپلیکیشن موبایل",
    description: "دریافت و راهنمای استفاده از اپلیکیشن موبایل.",
    isActive: false,
  },
  {
    id: "security",
    Icon: ShieldCheckIcon,
    title: "امنیت",
    description: "اطلاعات و راهکارهای افزایش امنیت حساب کاربری.",
    isActive: false,
  },
];

export function Menu() {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4 lg:gap-6">
      {items.map(({ id, isActive, Icon, title, description }) => (
        <Box
          component="a"
          href="#"
          key={id}
          className={clsx(
            "relative rounded-lg px-4 py-10 text-center",
            isActive
              ? "bg-primary-600 dark:bg-primary-500 text-white"
              : "shadow-soft dark:bg-dark-700 dark:text-dark-100 bg-white text-gray-800 dark:shadow-none",
          )}
        >
          <Icon
            className={clsx(
              "inline size-12",
              !isActive && "text-primary-600 dark:text-primary-400",
            )}
          />
          <h4 className="mt-2 text-lg font-medium">{title}</h4>
          <p className="mt-2 opacity-90">{description}</p>
        </Box>
      ))}
    </div>
  );
}
