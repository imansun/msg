...
function translatePermission(value: string): string {
  switch (value) {
    case "view":
      return "مشاهده";
    case "can-edit":
...
export function PermissionCell({
  ...
  return (
    ...
    >
      <span className="size-2 ml-1 rounded-full bg-current"></span>
      {translatePermission(row.original.permission.key)}
    </div>
  );
}

export function AssignCell({ getValue }: { getValue: Getter<number> }) {
  return <span>{getValue()} نفر</span>;
}
