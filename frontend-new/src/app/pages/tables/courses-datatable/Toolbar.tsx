                ...
                <MenuItems className="dark:border-dark-500 dark:bg-dark-700 absolute z-100 mt-1.5 min-w-[10rem] rounded-lg border border-gray-300 bg-white py-1 shadow-lg shadow-gray-200/50 outline-hidden focus-visible:outline-hidden ltr:right-0 rtl:left-0 dark:shadow-none">
                  ...
                  <MenuItem>
                    {({ focus }) => (
                      ...
                      >
                        <span>اشتراک‌گذاری سفارشات</span>
                      </button>
                    )}
                  </MenuItem>
...
function SearchInput({ table }: { table: Table<Course> }) {
  return (
    <Input
      ...
      placeholder="جستجو دوره، دسته‌بندی..."
      classNames={{
        root: "shrink-0",
      ...
      {toolbarFilters.includes("earning") && table.getColumn("earning") && (
        <div style={{ order: toolbarFilters.indexOf("earning") + 1 }}>
          <RangeFilter
            ...
            buttonText={({ min, max }) => (
              <>
                {min && <>از {formatNumber(min)} تومان</>}
                {min && max && " - "}
                {max && <>تا {formatNumber(max)} تومان</>}
              </>
            )}
            Icon={TbCurrencyDollar}
            MinPrefixIcon={TbCurrencyDollar}
      ...
      {toolbarFilters.includes("students") && table.getColumn("students") && (
        <div style={{ order: toolbarFilters.indexOf("students") + 1 }}>
          <RangeFilter
            ...
            buttonText={({ min, max }) => (
              <>
                {min && <>از {formatNumber(min)} دانشجو</>}
                {min && max && " - "}
                {max && <>تا {formatNumber(max)} دانشجو</>}
              </>
            )}
          />
        </div>
